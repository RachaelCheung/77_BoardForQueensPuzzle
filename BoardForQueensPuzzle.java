...
public class BoardForQueensPuzzle {
    ...
    public boolean lastIsNg() {
        for (int rankIndex = 0; rankIndex < lastRankFilled; rankIndex++){
			if (filesWithQueens[rankIndex] == filesWithQueens[lastRankFilled]){
				return true;
			}
			else if (Math.abs(rankIndex - lastRankFilled) == 
					Math.abs(filesWithQueens[rankIndex] - filesWithQueens[lastRankFilled])){
					return true;
			}
		}
		return false;
    ...
    public boolean accept() {
        return !lastIsNg() && (lastRankFilled == filesWithQueens.length - 1);
    }
